...
def check_slither_installation() -> Dict[str, Any]:
    ...
    try:
        ...
        # Check solcjs (installed via npm) first
        process_solc = subprocess.run(["solcjs", "--version"], capture_output=True, text=True, timeout=30, check=False)
        if process_solc.returncode == 0:
            result['solc_installed'] = True
            result['solc_version'] = process_solc.stdout.strip()
            result['solc_type'] = 'solcjs (npm)'
        else:
            # If solcjs fails, try solc (direct installation)
            process_solc = subprocess.run(["solc", "--version"], capture_output=True, text=True, timeout=30, check=False)
            if process_solc.returncode == 0:
                result['solc_installed'] = True
                result['solc_version'] = process_solc.stdout.strip()
                result['solc_type'] = 'solc (native)'
            else:
                result['errors'].append(f"Neither solcjs nor solc found. Please install solcjs using 'npm install -g solc' or install solc directly.")
