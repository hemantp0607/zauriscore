...
def _get_solc_version() -> str:
    try:
        # First try solcjs (installed via npm)
        proc = subprocess.run(["solcjs", "--version"], capture_output=True, text=True, timeout=10)
        if proc.returncode == 0:
            out = (proc.stdout or proc.stderr or "").strip()
            # Extract version like 0.8.30
        ...
        logging.warning(f"Could not determine solc version: {e}")
        return "unknown"
